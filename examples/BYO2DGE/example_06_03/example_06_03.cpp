...
//            go->update(input);
        ...
        unsigned status = mCamera->collideWCBound(mHero->getXform(), 0.8);
        std::string statusMsg;
        if (status & eBoundCollideStatus::eCollideLeft) {
            statusMsg += " LEFT";
        }
        if (status & eBoundCollideStatus::eCollideRight) {
            statusMsg += " RIGHT";
        }
        if (status & eBoundCollideStatus::eCollideBottom) {
            statusMsg += " BOTTOM";
        }
        if (status & eBoundCollideStatus::eCollideTop) {
            statusMsg += " TOP";
        }
        ...
        mMsg->setText(msg + mMode + " [Hero bound=" + statusMsg + "]");
    }
