...
public:
    void initialize(int, int, std::shared_ptr<Engine> engine) override {
        ...
        mMsg->setColor(glm::vec4(1.0f, 0.0f, 1.0f, 1.0f));
    ...
    void update(const Mn::Input &input, double) override {
        float deltaAmbient = 0.01;
        std::string msg = "Current Ambient: ";
        ...
        mCamera->panWith(mHero->getXform(), 0.8f);

        glm::vec4 v(0.3f, 0.3f, 0.3f, 1.0f);
        ...
        msg += " Red=" + std::to_string(v.x) + " Intensity=" + std::to_string(input.get_mouse_y_screen_position());
        mMsg->setText(msg);
    }
