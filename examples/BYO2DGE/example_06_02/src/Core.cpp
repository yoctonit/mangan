...
FontRenderable Core::createFontRenderable(const std::string &fontName, const std::string &text) const {
    const Font *font = getFont(fontName);
    SpriteRenderable spriteRenderable{
            mSpriteVao,
            mSpriteShader,
            font->getTexture(),
            mSpriteTextureBuffer
    };
    return {font, spriteRenderable, text};
}

void Core::addFont(const std::string &fontName) {
    auto entry = mFonts.find(fontName);
    if (entry != mFonts.end()) {
        std::cerr << "Font " << fontName << " already loaded\n";
        return;
    }
    mFonts.insert({fontName, Font{fontName}});
}

const Font *Core::getFont(const std::string &fontName) const {
    auto entry = mFonts.find(fontName);
    if (entry == mFonts.end()) {
        std::cerr << "Font " << fontName << " does not exists in engine\n";
    }
    return &(entry->second);
}
