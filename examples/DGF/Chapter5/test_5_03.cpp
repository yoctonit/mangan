...
public:
    void initialize() override {
        ...
        auto waveMaterial = std::make_shared<Material>("shader/Test_5_03.vert", "shader/Test_5_03.frag");
        waveMaterial->addUniform("tex", gridTexture.textureRef(), 1);
        waveMaterial->addUniform("time", 0.0f);

        m_mesh = std::make_shared<Mesh>("object", geometry, waveMaterial);
        m_scene->add(m_mesh);
    }
