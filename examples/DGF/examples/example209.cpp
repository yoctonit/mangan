...
public:
    ...
    void update() override {
        // update data
        // baseColor.Vector()[0] = static_cast<float>((sin(3.0 * (_time)) + 1.0) / 2.0);

        baseColor.Vector()[0] = static_cast<float>((sin(_time) + 1.0) / 2.0);
        baseColor.Vector()[1] = static_cast<float>((sin(_time + 2.1) + 1.0) / 2.0);
        baseColor.Vector()[2] = static_cast<float>((sin(3.0 * (_time + 4.2)) + 1.0) / 2.0);
