...
public:
    ...
    void update() override {
        // update data
        auto distance = static_cast<float>(m_speedPerSecond * m_deltaTimeSeconds);
        if (m_input.isPressedKey(MN_KEY_LEFT))
            m_translation.data()[0] -= distance;
        if (m_input.isPressedKey(MN_KEY_RIGHT))
            m_translation.data()[0] += distance;
        if (m_input.isPressedKey(MN_KEY_DOWN))
            m_translation.data()[1] -= distance;
        if (m_input.isPressedKey(MN_KEY_UP))
            m_translation.data()[1] += distance;
