...
public:
    ...
    void Update(const Mn::Input &input) {
        ...
        materialAmbientUniform = glm::vec3( 1.0f, 0.5f, 0.31f);
        materialDiffuseUniform = glm::vec3( 1.0f, 0.5f, 0.31f);
        materialSpecularUniform = glm::vec3( 0.5f, 0.5f, 0.5f);
        ...
        glm::vec3 lightColor;
        lightColor.x = static_cast<float>(std::sin(time * 2.0));
        lightColor.y = static_cast<float>(std::sin(time * 0.7));
        lightColor.z = static_cast<float>(std::sin(time * 1.3));
        glm::vec3 diffuseColor = lightColor * glm::vec3(0.5f); // decrease the influence
        glm::vec3 ambientColor = diffuseColor * glm::vec3(0.2f); // low influence

        lightAmbientUniform = ambientColor;
        lightDiffuseUniform = diffuseColor;
        lightSpecularUniform = glm::vec3(1.0f, 1.0f, 1.0f);

        viewPosUniform = camera.Position;

        // uniform for light source cube
        lightColorUniform = lightColor;
    }
