...
int main() {
    ...
    Mn::Shader shader(
            "shader/gs_default.vs",
            "shader/gs_default.fs"
    );
    Mn::Shader normalShader(
            "shader/normal_visualization.vs",
            "shader/normal_visualization.fs",
            "shader/normal_visualization.gs"
    );
    ...
    Mn::Vbo buffer1{box.Data(Mn::Geometry::Type::PositionsAndNormals)};
    Mn::Vao vao1{true};
    vao1.Connect(buffer1, 0, 3, 6, 0);
    vao1.Connect(buffer1, 1, 3, 6, 3);

    Mn::Uniform uModelNormal{normalShader.Locate("model"), GL_FLOAT_MAT4};
    Mn::Uniform uProjectionNormal{normalShader.Locate("projection"), GL_FLOAT_MAT4};
    Mn::Uniform uViewNormal{normalShader.Locate("view"), GL_FLOAT_MAT4};

    uTex = 0;

    glm::mat4 projection = glm::perspective(glm::radians(45.0f), (float) SCR_WIDTH / (float) SCR_HEIGHT, 1.0f, 100.0f);
    uProjection = projection;
    uProjectionNormal = projection;
    ...
    while (wnd.IsOpen() && runScene) {
        ...
        normalShader.Use();
        uModelNormal.Upload();
        uProjectionNormal.Upload();
        uViewNormal.Upload();
        vao1.Draw(GL_TRIANGLES, 0, 36);
